...
const navItems: Array<{ id: View; label: string; icon: IconComponent; helper: string }> = [
  { id: "pending", label: "Review queue", icon: LayoutDashboard, helper: "Matched jobs" },
  { id: "saved", label: "Saved roles", icon: Star, helper: "Shortlist" },
  { id: "search", label: "Add jobs", icon: Search, helper: "Manual intake" },
  { id: "profile", label: "Profile", icon: UserRound, helper: "Preferences" },
  { id: "tracker", label: "Pipeline", icon: ClipboardCheck, helper: "Applications" }
];
...
function App() {
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top_left,_rgba(99,102,241,0.25),_transparent_34rem),linear-gradient(135deg,#f8fafc,#eef2ff_45%,#f8fafc)] text-slate-950 transition-colors dark:bg-[radial-gradient(circle_at_top_left,_rgba(99,102,241,0.28),_transparent_34rem),linear-gradient(135deg,#020617,#0f172a_45%,#111827)] dark:text-white">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-[1500px] flex-col gap-4 p-3 lg:flex-row lg:p-5">
        <Sidebar view={view} setView={setView} profile={dashboard?.profile} jobs={jobs} />
        <main className="flex min-w-0 flex-1 flex-col gap-4">
          <TopBar query={query} setQuery={setQuery} theme={theme} setTheme={setTheme} onAddJob={() => setView("search")} profile={dashboard?.profile} />
          ...
          {isLoading ? (
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <HeroPanel profile={dashboard.profile} pendingCount={pendingJobs.length} avgMatch={avgMatch} />
                ...
                {view === "profile" && <ProfileEditor profileDraft={profileDraft} setProfileDraft={setProfileDraft} titlesDraft={titlesDraft} setTitlesDraft={setTitlesDraft} skillsDraft={skillsDraft} setSkillsDraft={setSkillsDraft} saveProfile={saveProfile} isBusy={isBusy} />}
...
function Sidebar({ view, setView, profile, jobs }: { view: View; setView: (view: View) => void; profile?: UserProfile; jobs: JobPosting[] }) {
  return (
    <motion.aside initial={{ opacity: 0, x: -24 }} animate={{ opacity: 1, x: 0 }} className="sticky top-5 z-20 flex h-fit flex-col gap-4 rounded-[2rem] border border-white/70 bg-white/70 p-3 shadow-2xl shadow-slate-950/[0.08] backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/60 dark:shadow-black/25 lg:h-[calc(100vh-2.5rem)] lg:w-80">
      <div className="flex items-center gap-3 rounded-[1.5rem] bg-slate-950 p-4 text-white dark:bg-white dark:text-slate-950">
        <div className="grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br from-indigo-400 via-violet-400 to-fuchsia-400 text-white shadow-lg shadow-indigo-500/30"><Sparkles className="h-5 w-5" /></div>
      ...
      <div className="flex gap-2 overflow-x-auto lg:grid lg:overflow-visible">
        {navItems.map((item) => {
          ...
          return (
            <button key={item.id} onClick={() => setView(item.id)} className="group relative flex min-w-[180px] items-center gap-3 rounded-2xl px-4 py-3 text-left transition hover:bg-slate-950/5 dark:hover:bg-white/10 lg:min-w-0">
              ...
              <span className="relative grid h-10 w-10 place-items-center rounded-xl bg-slate-950/5 text-slate-600 transition group-hover:scale-105 dark:bg-white/10 dark:text-slate-300"><Icon className="h-4 w-4" /></span>
              <span className="relative min-w-0"><span className="block truncate text-sm font-semibold text-slate-900 dark:text-white">{item.label}</span><span className="block truncate text-xs text-slate-500 dark:text-slate-400">{item.helper}</span></span>
            </button>
          );
        })}
      </div>
      ...
      <div className="mt-auto hidden rounded-[1.5rem] border border-white/70 bg-white/60 p-4 dark:border-white/10 dark:bg-white/[0.06] lg:block">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 text-center"><div className="rounded-2xl bg-slate-950/5 p-3 dark:bg-white/10"><p className="text-lg font-semibold">{jobs.length}</p><p className="text-[11px] text-slate-500 dark:text-slate-400">Total roles</p></div><div className="rounded-2xl bg-slate-950/5 p-3 dark:bg-white/10"><p className="text-lg font-semibold">{jobs.filter((job) => job.redFlags.length === 0).length}</p><p className="text-[11px] text-slate-500 dark:text-slate-400">Clean fits</p></div></div>
      </div>
    </motion.aside>
  );
}

function TopBar({ query, setQuery, theme, setTheme, onAddJob, profile }: { query: string; setQuery: (query: string) => void; theme: Theme; setTheme: (theme: Theme) => void; onAddJob: () => void; profile?: UserProfile }) {
  return (
    <motion.header initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} className="sticky top-3 z-10 flex flex-col gap-3 rounded-[2rem] border border-white/70 bg-white/75 p-3 shadow-xl shadow-slate-950/[0.06] backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/60 md:flex-row md:items-center">
      <div className="relative flex-1"><Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search jobs, skills, companies, locations..." className="h-12 rounded-full border-transparent bg-slate-950/[0.04] pl-11 shadow-none dark:bg-white/[0.08]" /><div className="pointer-events-none absolute right-4 top-1/2 hidden -translate-y-1/2 items-center gap-1 rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs text-slate-400 dark:border-white/10 dark:bg-slate-900 md:flex"><Command className="h-3 w-3" /> K</div></div>
      <div className="flex items-center gap-2"><Button variant="secondary" size="icon" onClick={() => setTheme(theme === "dark" ? "light" : "dark")} aria-label="Toggle theme">{theme === "dark" ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}</Button><Button variant="secondary" size="icon" aria-label="Notifications"><Bell className="h-4 w-4" /></Button><Button variant="gradient" onClick={onAddJob}><Plus className="h-4 w-4" /> Add job</Button><div className="hidden items-center gap-2 rounded-full border border-white/70 bg-white/70 py-1 pl-1 pr-3 dark:border-white/10 dark:bg-white/10 sm:flex"><Avatar name={profile?.name ?? "Candidate"} small /><span className="text-sm font-semibold text-slate-700 dark:text-slate-200">{profile?.name?.split(" ")[0] ?? "You"}</span></div></div>
    </motion.header>
  );
}

function HeroPanel({ profile, pendingCount, avgMatch }: { profile: UserProfile; pendingCount: number; avgMatch: number }) {
  return (
    <Card className="relative overflow-hidden border-white/80 bg-slate-950 text-white dark:bg-white/[0.06]"><div className="absolute inset-0 bg-[radial-gradient(circle_at_15%_20%,rgba(99,102,241,0.65),transparent_28rem),radial-gradient(circle_at_80%_0%,rgba(236,72,153,0.4),transparent_24rem)]" /><div className="absolute right-8 top-8 hidden h-56 w-56 rounded-full border border-white/10 bg-white/10 blur-3xl md:block" /><CardContent className="relative grid gap-6 p-6 md:grid-cols-[1fr_320px] md:p-8"><div><Badge className="border-white/20 bg-white/10 text-white">AI SaaS-style job command center</Badge><h1 className="mt-5 max-w-4xl text-4xl font-semibold tracking-[-0.07em] text-white md:text-6xl">Find test engineering roles with a sharper, calmer workflow.</h1><p className="mt-5 max-w-2xl text-base leading-7 text-white/70 md:text-lg">Built for test engineer, technician, quality, validation, and automotive testing searches. Paste roles from anywhere, review the evidence, and generate documents only when you approve.</p><div className="mt-6 flex flex-wrap gap-3"><Badge className="border-emerald-300/20 bg-emerald-400/10 text-emerald-100"><CheckCircle2 className="mr-1 h-3 w-3" /> Review before apply</Badge><Badge className="border-cyan-300/20 bg-cyan-400/10 text-cyan-100"><Target className="mr-1 h-3 w-3" /> {avgMatch}% avg match</Badge><Badge className="border-fuchsia-300/20 bg-fuchsia-400/10 text-fuchsia-100"><Sparkles className="mr-1 h-3 w-3" /> Tailored docs</Badge></div></div><div className="rounded-[1.5rem] border border-white/15 bg-white/10 p-5 backdrop-blur-xl"><p className="text-sm text-white/60">Today&apos;s focus</p><div className="mt-4 flex items-center justify-between"><div><p className="text-5xl font-semibold tracking-[-0.08em]">{pendingCount}</p><p className="text-sm text-white/60">roles in review</p></div><MatchRing score={avgMatch} inverse /></div><div className="mt-6 rounded-2xl bg-white/10 p-4"><p className="text-sm font-semibold">{profile.preferredTitles[0] ?? "Test Engineer"}</p><p className="mt-1 text-xs leading-5 text-white/60">Targeting {profile.location} within {profile.commuteMiles} miles and ${profile.desiredPayMin}-${profile.desiredPayMax}/hr.</p></div></div></CardContent></Card>
  );
}
...
function AnalyticsGrid({ analytics }: { analytics: Array<{ label: string; value: string | number; helper: string; icon: IconComponent; gradient: string }> }) {
  return <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{analytics.map((item, index) => { const Icon = item.icon; return <motion.div key={item.label} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}><Card className="group overflow-hidden"><CardContent className="p-5"><div className="flex items-center justify-between"><div className={`grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br ${item.gradient} text-white shadow-lg shadow-indigo-500/15`}><Icon className="h-5 w-5" /></div><TrendingUp className="h-4 w-4 text-emerald-500 opacity-70 transition group-hover:scale-110" /></div><p className="mt-5 text-3xl font-semibold tracking-[-0.06em] text-slate-950 dark:text-white">{item.value}</p><p className="mt-1 text-sm font-medium text-slate-600 dark:text-slate-300">{item.label}</p><p className="mt-2 text-xs leading-5 text-slate-500 dark:text-slate-500">{item.helper}</p></CardContent></Card></motion.div>; })}</div>;
}
...
function ApplicationTracker({ jobs, onGenerate, onStatus, progressPercent }: { jobs: JobPosting[]; onGenerate: (job: JobPosting) => void; onStatus: (job: JobPosting, status: ApplicationStatus) => void; progressPercent: number }) {
  return <div className="grid gap-4"><Card><CardContent className="p-6"><div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div><CardTitle className="text-2xl">Application pipeline</CardTitle><CardDescription>Move jobs from saved to applied, interview, offer, or rejected as your search progresses.</CardDescription></div><div className="min-w-[220px]"><div className="flex justify-between text-sm font-medium text-slate-600 dark:text-slate-300"><span>Pipeline progress</span><span>{progressPercent}%</span></div><div className="mt-2 h-3 overflow-hidden rounded-full bg-slate-200 dark:bg-white/10"><motion.div className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-fuchsia-500" initial={{ width: 0 }} animate={{ width: `${progressPercent}%` }} /></div></div></div></CardContent></Card><div className="grid gap-4 overflow-x-auto lg:grid-cols-5">{statusOrder.filter((status) => status !== "Pending").map((status) => <Card key={status} className="min-h-[260px] min-w-[220px]"><CardHeader className="p-4"><CardTitle className="text-base">{status}</CardTitle></CardHeader><CardContent className="space-y-3 p-4 pt-0">{jobs.filter((job) => job.status === status).length === 0 && <p className="rounded-2xl border border-dashed border-slate-200 p-4 text-sm text-slate-400 dark:border-white/10">No roles yet.</p>}{jobs.filter((job) => job.status === status).map((job) => <div key={job.id} className="rounded-2xl bg-slate-950/[0.04] p-3 dark:bg-white/[0.06]"><p className="font-semibold text-slate-900 dark:text-white">{job.title}</p><p className="text-sm text-slate-500 dark:text-slate-400">{job.company}</p><div className="mt-3 flex gap-2"><Button variant="secondary" size="sm" onClick={() => onGenerate(job)}>Docs</Button>{status !== "Interview" && <Button variant="ghost" size="sm" onClick={() => onStatus(job, "Interview")}>Interview</Button>}</div></div>)}</CardContent></Card>)}</div></div>;
}
...
function ApplicationDialog({ generated, activeJob, setGenerated, onApplied }: { generated: GeneratedApplication | null; activeJob: JobPosting | null; setGenerated: (generated: GeneratedApplication | null) => void; onApplied: (job: JobPosting) => void }) {
  ...
  return <Dialog open={Boolean(generated && activeJob)} onOpenChange={(open) => !open && setGenerated(null)}><DialogContent>{generated && activeJob && <div className="max-h-[90vh] overflow-y-auto p-6 md:p-8"><DialogHeader className="pr-10"><Badge className="w-fit bg-amber-500/10 text-amber-600 dark:text-amber-300">Manual review required</Badge><DialogTitle>Application package for {activeJob.company}</DialogTitle><DialogDescription>Review every line before submitting. The app will not apply for you automatically.</DialogDescription></DialogHeader><div className="mt-6 grid gap-4 lg:grid-cols-2"><DocumentBox title="Tailored resume draft" value={generated.resume} onCopy={() => copy(generated.resume, "Resume")} /><DocumentBox title="Cover letter draft" value={generated.coverLetter} onCopy={() => copy(generated.coverLetter, "Cover letter")} /></div><div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-400/20 dark:bg-amber-400/10"><p className="font-semibold text-amber-900 dark:text-amber-100">Pre-submit checklist</p><ul className="mt-2 space-y-2 text-sm text-amber-800 dark:text-amber-100/80">{generated.checklist.map((item) => <li key={item}>- {item}</li>)}</ul></div><div className="mt-6 flex flex-wrap justify-end gap-3"><Button variant="secondary" onClick={() => setGenerated(null)}>Keep reviewing</Button><Button variant="gradient" onClick={() => onApplied(activeJob)}><CheckCircle2 className="h-4 w-4" /> I submitted it, mark applied</Button></div></div>}</DialogContent></Dialog>;
}

function OnboardingDialog({ open, onComplete }: { open: boolean; onComplete: () => void }) {
  return <Dialog open={open} onOpenChange={(next) => !next && onComplete()}><DialogContent className="max-w-3xl"><div className="p-6 md:p-8"><DialogHeader><Badge className="w-fit bg-indigo-500/10 text-indigo-600 dark:text-indigo-300">Welcome</Badge><DialogTitle className="text-3xl">Your test engineering job search assistant is ready.</DialogTitle><DialogDescription>Start with demo matches, paste your own job descriptions, and approve everything before applying.</DialogDescription></DialogHeader><div className="mt-6 grid gap-3 md:grid-cols-3">{[{ icon: UserRound, title: "Tune profile", text: "Set roles, skills, pay, location, resume, and tone." }, { icon: Search, title: "Paste jobs", text: "Use any source instead of relying only on LinkedIn." }, { icon: Wand2, title: "Generate docs", text: "Create drafts, review, then submit manually." }].map((item) => { const Icon = item.icon; return <div key={item.title} className="rounded-2xl bg-slate-950/[0.04] p-4 dark:bg-white/[0.06]"><Icon className="h-6 w-6 text-indigo-500" /><p className="mt-3 font-semibold">{item.title}</p><p className="mt-1 text-sm leading-5 text-slate-500 dark:text-slate-400">{item.text}</p></div>; })}</div><Button className="mt-6 w-full" variant="gradient" size="lg" onClick={onComplete}>Enter workspace <ArrowRight className="h-4 w-4" /></Button></div></DialogContent></Dialog>;
}

function DocumentBox({ title, value, onCopy }: { title: string; value: string; onCopy: () => void }) {
  return <div className="rounded-[1.25rem] border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/[0.04]"><div className="flex items-center justify-between gap-3"><h3 className="font-semibold">{title}</h3><Button variant="secondary" size="sm" onClick={onCopy}>Copy</Button></div><Textarea value={value} readOnly rows={18} className="mt-3 font-mono text-xs leading-5" /></div>;
}

function MatchRing({ score, inverse = false }: { score: number; inverse?: boolean }) {
  const circumference = 2 * Math.PI * 26;
  const offset = circumference - (score / 100) * circumference;
  return <div className="relative grid h-20 w-20 place-items-center"><svg className="h-20 w-20 -rotate-90" viewBox="0 0 64 64"><circle cx="32" cy="32" r="26" fill="none" stroke={inverse ? "rgba(255,255,255,0.18)" : "currentColor"} strokeWidth="7" className={inverse ? "" : "text-slate-200 dark:text-white/10"} /><motion.circle cx="32" cy="32" r="26" fill="none" stroke="url(#scoreGradient)" strokeLinecap="round" strokeWidth="7" strokeDasharray={circumference} initial={{ strokeDashoffset: circumference }} animate={{ strokeDashoffset: offset }} transition={{ duration: 0.8, ease: "easeOut" }} /><defs><linearGradient id="scoreGradient" x1="0" x2="1"><stop stopColor="#6366f1" /><stop offset="1" stopColor="#ec4899" /></linearGradient></defs></svg><span className={`absolute text-sm font-bold ${inverse ? "text-white" : "text-slate-950 dark:text-white"}`}>{score}%</span></div>;
}
...
function EmptyState({ icon: Icon, title, description, action }: { icon: IconComponent; title: string; description: string; action?: React.ReactNode }) {
  return <div className="grid place-items-center rounded-[1.5rem] border border-dashed border-slate-300 bg-white/40 p-10 text-center dark:border-white/10 dark:bg-white/[0.03]"><div className="grid h-14 w-14 place-items-center rounded-2xl bg-slate-950/5 text-slate-500 dark:bg-white/10 dark:text-slate-300"><Icon className="h-6 w-6" /></div><h3 className="mt-4 text-lg font-semibold">{title}</h3><p className="mt-2 max-w-md text-sm leading-6 text-slate-500 dark:text-slate-400">{description}</p>{action && <div className="mt-5">{action}</div>}</div>;
}

function DashboardSkeleton() {
  return <div className="grid gap-4"><Skeleton className="h-80 rounded-[2rem]" /><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><Skeleton className="h-40" /><Skeleton className="h-40" /><Skeleton className="h-40" /><Skeleton className="h-40" /></div><Skeleton className="h-96 rounded-[2rem]" /></div>;
}

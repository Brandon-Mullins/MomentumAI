...
function ApplicationDialog({ generated, activeJob, profile, setGenerated, onApplied }: { generated: GeneratedApplication | null; activeJob: JobPosting | null; profile: UserProfile | null; setGenerated: (generated: GeneratedApplication | null) => void; onApplied: (job: JobPosting) => void }) {
  ...
  const [template, setTemplate] = useState<ResumeTemplate>("ATS Clean");
  ...
  const exportZip = async () => {
    ...
    zip.file("README.txt", "MomentumAI Professional Resume Studio\n\nFiles included:\n- resume.tex: Overleaf-ready resume source\n- coverletter.tex: Overleaf-ready cover letter source\n- resume.txt: editable plain-text resume\n- coverletter.txt: editable plain-text cover letter\n\nPDF exports are available from the MomentumAI Export Center. Browser ZIP generation does not embed PDF binaries in this MVP. Review every generated line for accuracy. Do not claim experience, tools, or credentials you do not have.");
...
function ResumePreview({ studio, activeJob, pageLength }: { studio: ReturnType<typeof buildResumeStudio>; activeJob: JobPosting; pageLength: ResumePageLength }) {
  const [header, contact, , , summary, , , ...rest] = studio.resumePlainText.split("\n");
  const skills = studio.resumePlainText.match(/CORE SKILLS\n([\s\S]*?)\n\nEXPERIENCE/)?.[1]?.split("\n").map((item) => item.replace(/^-\s*/, "")).filter(Boolean) ?? [];
  const experience = studio.resumePlainText.match(/EXPERIENCE\n([\s\S]*?)\n\nTOOLS/)?.[1]?.split("\n").map((item) => item.replace(/^-\s*/, "")).filter(Boolean) ?? [];
  return <div className="mt-4 grid gap-4 xl:grid-cols-[1fr_320px]"><div className="rounded-[1.25rem] bg-slate-200 p-4 dark:bg-slate-900"><div className="mx-auto min-h-[760px] max-w-[760px] bg-white p-8 text-slate-950 shadow-2xl ring-1 ring-slate-200"><div className="border-b-4 border-indigo-600 pb-4"><h2 className="text-center font-serif text-3xl font-bold tracking-tight">{header}</h2><p className="mt-2 text-center text-sm text-slate-600">{contact}</p><p className="mt-1 text-center text-xs font-semibold uppercase tracking-[0.24em] text-indigo-700">Target: {activeJob.title}</p></div><section className="mt-5"><h3 className="border-b border-slate-300 pb-1 text-sm font-bold uppercase tracking-[0.18em] text-indigo-700">Professional Summary</h3><p className="mt-2 text-sm leading-6">{summary}</p></section><section className="mt-5"><h3 className="border-b border-slate-300 pb-1 text-sm font-bold uppercase tracking-[0.18em] text-indigo-700">Core Skills</h3><div className="mt-2 grid grid-cols-2 gap-x-6 gap-y-1 text-sm">{skills.slice(0, pageLength === "one-page" ? 10 : 14).map((skill) => <span key={skill}>• {skill}</span>)}</div></section><section className="mt-5"><h3 className="border-b border-slate-300 pb-1 text-sm font-bold uppercase tracking-[0.18em] text-indigo-700">Experience Highlights</h3><div className="mt-2"><p className="font-bold">Verified Experience <span className="font-normal text-slate-500">| Tailored for {activeJob.company}</span></p><ul className="mt-1 list-disc space-y-1 pl-5 text-sm leading-5">{experience.map((line) => <li key={line}>{line}</li>)}</ul></div></section><section className="mt-5 grid grid-cols-2 gap-6"><div><h3 className="border-b border-slate-300 pb-1 text-sm font-bold uppercase tracking-[0.18em] text-indigo-700">Tools / Projects</h3><ul className="mt-2 list-disc space-y-1 pl-5 text-sm">{studio.projectsOrTools.length ? studio.projectsOrTools.map((tool) => <li key={tool}>{tool}</li>) : <li>Add verified tools or projects.</li>}</ul></div><div><h3 className="border-b border-slate-300 pb-1 text-sm font-bold uppercase tracking-[0.18em] text-indigo-700">Education</h3><ul className="mt-2 list-disc space-y-1 pl-5 text-sm">{studio.education.length ? studio.education.map((item) => <li key={item}>{item}</li>) : <li>Add verified education/certs.</li>}</ul></div></section>{pageLength === "two-page" && <><div className="my-8 border-t border-dashed border-slate-300 text-center text-xs text-slate-400">Page break</div><pre className="whitespace-pre-wrap font-serif text-sm leading-6">{studio.coverLetterPlainText}</pre></>}</div></div><div className="grid gap-3"><Card><CardHeader><CardTitle>Why sections were included</CardTitle><CardDescription>{activeJob.title} at {activeJob.company}</CardDescription></CardHeader><CardContent className="space-y-3">{studio.includedSections.map((item) => <div key={item.section} className="rounded-2xl bg-slate-950/[0.04] p-3 dark:bg-white/[0.06]"><p className="font-semibold">{item.section}</p><p className="text-sm text-slate-500 dark:text-slate-400">{item.why}</p></div>)}</CardContent></Card><OverleafInstructions /></div></div>;
}

function ResumeScorePanel({ studio }: { studio: ReturnType<typeof buildResumeStudio> }) {
  const metrics = [
    { label: "ATS Score", value: studio.score.atsScore },
    { label: "Readability", value: studio.score.readabilityScore },
    { label: "Match Score", value: studio.score.matchScore },
    { label: "Keyword Coverage", value: studio.score.keywordCoverage }
  ];
...
function ExportCenter({ studio, activeJob, onPdf, onDocx, onZip, onResumeTex, onCoverTex }: { studio: ReturnType<typeof buildResumeStudio>; activeJob: JobPosting; onPdf: () => void; onDocx: () => void; onZip: () => void; onResumeTex: () => void; onCoverTex: () => void }) {
  ...
  const actionFor = (kind: string) => kind === "pdf" ? onPdf : kind === "docx" ? onDocx : kind === "zip" ? onZip : () => { onResumeTex(); onCoverTex(); };
  return <div className="mt-4"><Card><CardHeader><CardTitle>Export center</CardTitle><CardDescription>Export professional materials for {activeJob.company}. PDF is rendered output; TEX is source code for Overleaf.</CardDescription></CardHeader><CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{options.map((item) => <button key={item.kind} onClick={actionFor(item.kind)} className="rounded-2xl border border-slate-200 bg-white/70 p-5 text-left transition hover:-translate-y-1 hover:border-indigo-300 dark:border-white/10 dark:bg-white/[0.06]"><p className="text-xl font-semibold">{item.label}</p><p className="mt-1 text-xs font-semibold text-indigo-500">{item.filename}</p><p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{item.description}</p></button>)}</CardContent></Card><OverleafInstructions /></div>;
}

function OverleafInstructions() {
  return <div className="rounded-2xl bg-slate-950/[0.04] p-4 text-sm text-slate-600 dark:bg-white/[0.06] dark:text-slate-300"><p className="font-semibold text-slate-900 dark:text-white">Open in Overleaf instructions</p><ol className="mt-2 list-decimal space-y-1 pl-5"><li>Download <code>resume.tex</code> or <code>coverletter.tex</code>.</li><li>Create a new Overleaf project and upload or paste the file.</li><li>Compile with <strong>pdfLaTeX</strong>.</li><li>Edit marked sections before submitting.</li></ol></div>;
}
